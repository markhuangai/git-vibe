...
- `next_state` should describe the next GitVibe workflow state from the stage schema, such as `ready-for-materialization`, `ready-for-implementation`, `implementation-issues-ready`, `review-passed`, `changes-required`, `needs-info`, or `blocked`.
