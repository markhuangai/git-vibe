...
export function acceptedRiskStageResultBody(
  ...
  return [
    ...
    "",
    "**Status:** `blocked`",
    "**Risk:** `accepted for matching context`",
    "",
    `${inlineCode(metadata.actor || "<unknown>")} accepted this prompt-injection input risk for matching \`${metadata.stage}\` context.`,
...
export function acceptedRiskMetadataBlock(metadata: AcceptedRiskMetadata): string {
  return [
    ...
    "### Accepted Risk",
    "",
    `${inlineCode(metadata.actor || "<unknown>")} accepted this prompt-injection input risk for matching context.`,
