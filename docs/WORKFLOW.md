...
flowchart TD
  subgraph ParentRun[Develop run for current issue]
    ...
    L --> M{Review result}
    M -->|changes required| N[Post review result and mark PR blocked]
...
`changes-required`, GitVibe posts the review result on the pull request and
marks the PR `gvi:blocked`. When review returns `review-passed`, GitVibe removes
stale blocked/reviewing state and marks the PR `gvi:ready-for-approval`.
